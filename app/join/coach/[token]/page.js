...
export default async function CoachInvitePage({ params, searchParams }) {
  ...
  const { data: invite } = await admin
    .from("coach_invites")
    .select("email, sport, accepted, clubs(name)")
    .eq("token", token)
    .single();
  ...
  return (
    <div className="min-h-screen bg-surface flex items-center justify-center p-6">
      <div className="bg-white border border-border rounded-2xl p-10 max-w-md w-full">
        ...
        <h1 className="text-2xl font-bold text-foreground mb-1">Set up your coach account</h1>
        <p className="text-sm text-muted mb-8">
          You've been invited by <strong>{invite.clubs?.name}</strong> as a coach for <strong>{invite.sport}</strong>.
        </p>
        ...
        <form action={activateCoachAccount} className="flex flex-col gap-4">

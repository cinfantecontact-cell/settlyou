...
export async function POST(request) {
  ...
  const { sport, disabled_base_docs, custom_docs } = await request.json();
  ...
  const { error } = await admin
    .from("sport_document_config")
    .upsert(
      { club_id: profile.club_id, sport: targetSport, disabled_base_docs, custom_docs, updated_at: new Date().toISOString() },

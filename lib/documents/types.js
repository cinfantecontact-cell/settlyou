 ...
 *
 * @param {object} sportConfig - Row from sport_document_config
 * @param {string|null} nationality - Athlete nationality (e.g. "US", "Brazil"). Null = show all.
 */
export function getSportDocTypes(sportConfig, nationality = null) {
  if (!sportConfig) return BASE_DOCUMENT_TYPES;

  const disabled = new Set(sportConfig.disabled_base_docs || []);
  const docSettings = sportConfig.doc_settings || {};

  const base = BASE_DOCUMENT_TYPES
    .filter(d => !disabled.has(d.key))
    .map(d => {
      const settings = docSettings[d.key] || {};
      return {
        ...d,
        visibility: settings.visibility || "all",
        order: settings.order != null ? settings.order : Infinity,
      };
    });
  ...
  const custom = (sportConfig.custom_docs || []).map(d => ({
    ...
    order: d.order != null ? d.order : Infinity,
  }));

  const all = [...base, ...custom].sort((a, b) => a.order - b.order);

  // Apply visibility filter when nationality is known
  if (!nationality) return all;
  const isDomestic = ["US", "UNITED STATES"].includes(nationality.trim().toUpperCase());
  return all.filter(d => {
    if (d.visibility === "all") return true;
    if (d.visibility === "international") return !isDomestic;
    if (d.visibility === "domestic") return isDomestic;
    return true;
  });
}

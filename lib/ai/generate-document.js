...
Rules:
- Use real, specific places in ${city} — no generic advice.
...
${r.additional_notes ? `STUDENT NOTES: ${r.additional_notes}` : ""}
${r.club_custom_notes ? `\nINSTITUTION MESSAGE (MANDATORY — see rules below):\n"${r.club_custom_notes}"` : ""}
...
${r.club_custom_notes ? `- INSTITUTION NOTES ARE MANDATORY: Copy the institution's message WORD FOR WORD into the "university_notes" field. Do NOT paraphrase, summarize, or omit any part. After the verbatim copy, add 2-3 sentences that expand on the key points with practical, specific detail the student can act on.` : ""}
- URLs: For every "url" field use a Google Maps search link: https://www.google.com/maps/search/PLACE+NAME+${encodeURIComponent(r.destination_city || "")}
- Tone: warm, direct, practical — like a senior advisor who knows the answer and gives it clearly.
- Use REAL places near ${r.university || r.destination_city}. Specific street addresses and names matter.
...
- Do NOT invent website URLs. Google Maps search links only.
...

{
  ...
  "meta": {
    ...
    "welcome_letter": "3-4 sentences personal welcome to ${r.athlete_name} — acknowledge their journey, express genuine excitement, and give them one practical tip for their first week"
  },
  "university_notes": ${r.club_custom_notes ? `"copy institution message verbatim here, then add 2-3 sentences of expanded practical detail"` : "null"},
  ...
  "sections": {
    ...
    "your_paperwork": {
      ...
      ${isAthlete ? `"eligibility": {
        "intro": "one sentence — what eligibility means and why it matters for ${r.division || "college athletics"}",
        "steps": [
          {"step": 1, "title": "", "description": "specific, actionable explanation of what to do, what form to use, and where to submit", "deadline": "deadline or null", "url": "official eligibility portal for ${r.division || "this league"} — e.g. https://web3.ncaa.org/ecwr3/ for NCAA, https://www.playnaia.org/page/eligibility.php for NAIA, https://www.njcaa.org/eligibility/index for NJCAA"}
        ]
      },
      "athletic_forms": [
        {"title": "Pre-Participation Physical / Medical Clearance", "description": "one sentence — what to submit, to whom, and by when before first practice", "url": "athletic department URL or null"},
        {"title": "NIL — Name, Image & Likeness", "description": "one sentence — what NIL means, what ${r.athlete_name} can and cannot do, and where to register with the school's NIL portal", "url": "https://www.ncaa.org/sports/2021/6/3/student-athletes-name-image-likeness.aspx"},
        {"title": "Drug Testing Consent & Emergency Forms", "description": "one sentence — drug testing consent, emergency contact form, and travel consent — all submitted to the athletic department before the first practice", "url": null}
      ],` : `"eligibility": null,
      "athletic_forms": null,`}
      "health_insurance": {
        "description": "one sentence — enroll in ${r.university || "the university"}'s student health insurance plan or submit a waiver with proof of your own qualifying coverage — missing the deadline means automatic enrollment and a charge to your student account",
        "deadline": "specific deadline if known, otherwise 'before semester start'",
        "url": "school student health insurance portal URL — use Google Maps search for '${r.university || r.destination_city} student health insurance' if unknown"
      },
      ${!isInternational ? `"financial_aid": [
        {"title": "Accept your financial aid award letter", "description": "one sentence — log into the student portal, review each type of aid (grants, scholarships, loans), and accept or decline by the deadline", "url": "https://studentaid.gov/"},
        {"title": "FAFSA renewal each October", "description": "one sentence — FAFSA must be renewed every year in October for the following academic year — set a reminder now", "url": "https://studentaid.gov/h/apply-for-aid/fafsa"}
      ],` : `"financial_aid": null,`}
      ${isInternational ? `"international_academic": {
        "intro": "one sentence — what this section covers and why it's critical to start early",
        "transcript_evaluation": [
          {"step": 1, "title": "", "description": "exactly what to do: which service to use, what documents to send, cost, and typical timeline", "url": "evaluation service URL — e.g. https://www.wes.org or https://www.ece.org"}
        ],
        "gpa_conversion": "one paragraph explaining how GPA conversion works for students from ${r.current_country || "their home country"} and which service to use",
        "required_vaccines": ["specific vaccine name and note on where to get it"],
        "vaccines_url": "official immunization requirements page for ${r.university || r.destination_city}",
        "pre_arrival_docs": ["specific document name and exactly what to do with it"]
      },
      "post_arrival": {
        "intro": "one sentence — the first things to do in the first 30 days after arriving",
        "i94_check": {"description": "one sentence — verify your I-94 arrival record was correctly recorded at i94.cbp.dhs.gov within the first 3 days; if it shows a wrong date or status, contact the international student office immediately", "url": "https://i94.cbp.dhs.gov"},
        "social_security": [
          {"step": 1, "title": "", "description": "exact office address, what to bring, what form, how long it takes", "url": null}
        ],
        "ssa_office_url": "Google Maps link to the nearest Social Security Administration office to ${r.university || r.destination_city}",
        "itin": {"description": "one sentence — if you cannot yet get an SSN, apply for an ITIN (IRS Form W-7) to file taxes and open certain bank accounts — apply through your international student office", "url": "https://www.irs.gov/individuals/individual-taxpayer-identification-number"},
        "bank_account": [
          {"step": 1, "title": "", "description": "specific bank near campus, what documents to bring, student account benefits, how long it takes", "url": null}
        ],
        "us_taxes": {"description": "one sentence — all F-1 students must file Form 8843 with the IRS every year even with zero income; if you have a scholarship or stipend, also file Form 1040-NR — use Sprintax which is built for international students", "url": "https://www.sprintax.com"},
        "opt_cpt": {"text": "2-3 sentences — what OPT and CPT are, when each applies, and how to apply through the international student office with the lead time required", "url": "international student office OPT/CPT page at ${r.university || r.destination_city}"},
        "cv_tips": ["one specific, actionable tip for building a US-style resume as an international student"],
        "resume_tools_url": "https://www.canva.com/resumes/"
      }` : `"international_academic": null,
      "post_arrival": {
        "intro": "one sentence — key things to sort in your first two weeks on campus",
        "i94_check": null,
        ...
        "state_id": {"description": "one sentence — if moving from another state, transfer your driver's license or get a new state ID at the local DMV within 60 days of establishing residency — bring proof of address and your current license", "url": "state DMV website URL for the state where ${r.destination_city} is located"}
      }`}
    }
  }
}`;
...
export async function generateBaseData(club) {
  ...
  return await callModel("claude-haiku-4-5-20251001", 16000, prompt);
}
...
export async function generateRelocationDocument(request, baseData) {
  ...
  if (request.athlete_type === "college") {
    console.log("[generate] using short guide prompt (schema_version 2)...");
    const prompt = buildShortGuidePrompt(request);
    return await callWithFallback(8000, prompt);
  }

  // Pro athletes: two-tier if base data available
  if (baseData) {
    console.log("[generate] using two-tier personalization...");
    return await callWithFallback(24000, buildProPersonalizationPrompt(request, baseData));
  }

  // Pro athletes: monolithic fallback
  console.log("[generate] no base data, using monolithic generation...");
  return await callWithFallback(32000, buildPrompt(request));
}

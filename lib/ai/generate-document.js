...
${r.club_custom_notes ? `- INSTITUTION NOTES — filter carefully before writing university_notes:
  • Notes about international students, DSO check-in, visa, SEVIS, immigration → only if IS INTERNATIONAL STUDENT: ${isInternational ? "Yes" : "No"}
  • Notes about student-athletes, NCAA/NAIA/NJCAA eligibility, compliance, training, sports medicine, team → only if IS STUDENT-ATHLETE: ${isAthlete ? "Yes" : "No"}
  • Notes that mention a specific sport by name (e.g. "soccer team", "basketball players", "swim team") → only include if that sport matches this student's sport (${isAthlete ? r.sport || "N/A" : "not applicable — not an athlete"}); if it names a different sport, skip it entirely
  • If this student IS BOTH international AND a student-athlete, include both layers — do not skip either
  • Notes about general campus, deadlines, contacts, welcome messages → always include
  • Read the INTENT of each note — "report to compliance before your first workout" is clearly athlete-only even without the word "athlete"
  • Write only the applicable notes in university_notes (preserve the original wording), then add 2-3 sentences of practical expanded detail
  • Do NOT include any prefix labels like "Men's Soccer Coach Notes:", "Coach Notes:", "Institution Notes:", or similar — write the content directly, no headers
  • If no institution notes apply to this student, set university_notes to null` : ""}
${r.club_custom_links?.length ? `- INSTITUTION LINKS — embed in relevant sections; remainder goes in university_links:
  • Use the EXACT provided URL on the most relevant section item covering that topic (overrides the Google Maps URL rule for institution links)
...

{
  ...
  "university_notes": ${r.club_custom_notes ? `"only the institution notes that apply to this student (preserve original wording), then 2-3 sentences of expanded practical detail — or null if none apply"` : "null"},

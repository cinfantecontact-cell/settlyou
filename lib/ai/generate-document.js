...
function repairTruncatedJson(str) {
  // Close any unterminated string by finding last complete key-value pair
  let s = str;
  // Remove trailing incomplete token (partial string, key, or value)
...
async function callModel(model, maxTokens, prompt, useWebSearch = false) {
  ...
  try {
    ...
    // If we hit the token limit, try to repair the truncated JSON
    if (response.stop_reason === "max_tokens") {
      console.warn("[generate] Hit max_tokens, attempting JSON repair...");
      try {
        const repaired = repairTruncatedJson(cleaned);
        return JSON.parse(repaired);
      } catch {
        // repair failed — fall through to throw
      }
    }
...
export async function generateBaseData(club) {
  ...
  return await callWithFallback(16000, prompt, false);
}
...
export async function generateRelocationDocument(request, baseData) {
  ...
  if (request.athlete_type === "college") {
    console.log("[generate] using short guide prompt (schema_version 2) with Sonnet, no web search...");

...
export async function sendAthleteReportReady({ athleteName, athleteEmail, clubName, reportToken, uploadToken }) {
  const resend = new Resend(process.env.RESEND_API_KEY);
  const reportUrl = `${process.env.NEXT_PUBLIC_APP_URL}/report/${reportToken}`;
  const uploadUrl = uploadToken ? `${process.env.NEXT_PUBLIC_APP_URL}/upload/${uploadToken}` : null;
